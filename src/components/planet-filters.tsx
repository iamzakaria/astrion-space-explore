...
const SORTS: { id: PlanetSort; label: string }[] = [
  { id: "au", label: "Orbit" },
  { id: "name", label: "Name" },
  { id: "mass", label: "Mass" },
...
interface Props {
  value: PlanetFilters;
  onChange: (next: PlanetFilters) => void;
  resultCount: number;
}

export function PlanetFiltersBar({ value, onChange, resultCount }: Props) {
  const set = (patch: Partial<PlanetFilters>) => onChange({ ...value, ...patch });
  ...
  return (
    <div className="space-y-5">
      ...
      <FilterRow label="Family">
        <Chip active={value.system === "solar"} onClick={() => set({ system: value.system === "solar" ? "" : "solar" })}>
          Solar System
        </Chip>
        <Chip
      ...
      <FilterRow label="Type">
        {PLANET_KIND_INFO.map((k) => (
          <Chip
            ...
            onClick={() => set({ kind: value.kind === k.id ? "" : (k.id as PlanetKind) })}
          >
            {k.label}
          </Chip>
        ))}
      ...
      <FilterRow label="Found by">
        {DISCOVERY_INFO.map((d) => (
          <Chip
            key={d.id}
            active={value.discovery === d.id}
            onClick={() =>
              set({ discovery: value.discovery === d.id ? "" : (d.id as DiscoveryMethod) })
            }
          >
            {d.label}
          </Chip>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="font-mono text-xs tracking-wide text-muted tabular">
          {resultCount} {resultCount === 1 ? "world" : "worlds"}
        </p>
        <div className="flex flex-wrap items-center gap-2">
          <label className="flex items-center gap-2 text-xs text-muted">
            ...
            <select
              value={value.sort}
              onChange={(e) => set({ sort: e.target.value as PlanetSort })}
          ...
          {hasFilters ? (
            <button
              ...
              onClick={() =>
                onChange({ ...value, q: "", kind: "", system: "", notable: "", discovery: "" })
              }
            >
              Clear filters
            </button>
